...
def register_for_event(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            # Create the registration object without saving to DB yet
            registration = form.save(commit=False)
            
            # Set the logged-in user to the registration object
            registration.user = request.user  # Associate the logged-in user
...
def join_waiting_list(request, event_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            waiting_list_entry.save()  # Now save it
