...
def join_waiting_list(request, event_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            waiting_list_entry.event = event  # Associate the event with the waiting list entry
            waiting_list_entry.user = request.user  # Set the logged-in user
            waiting_list_entry.save()  # Save the waiting list entry
