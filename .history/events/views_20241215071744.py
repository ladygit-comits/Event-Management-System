...
def user_dashboard(request):
    # Use user foreign key instead of email to filter registrations and waiting list
    user = request.user  # Get the current logged-in user
    
    # Filter user's registrations and waiting list entries using the user foreign key
    user_registrations = Registration.objects.filter(user=user)
    user_waiting_list = WaitingList.objects.filter(user=user)
