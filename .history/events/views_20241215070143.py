...
def register_event(request, event_id):
    ...
    if request.method == "POST":
        form = RegistrationForm(request.POST, user=request.user)  # Pass logged-in user
        if form.is_valid():
            registration = form.save(commit=False)
            registration.user = request.user  # Ensure user is assigned
            registration.event = event
            registration.save()
            return redirect('user_dashboard')  # Redirect after registration
...
def join_waiting_list(request, event_id):
    event = get_object_or_404(Event, id=event_id)  # Ensure we get the event by its ID
    
    if request.method == 'POST':
        form = WaitingListForm(request.POST, user=request.user)  # Pass logged-in user to form
        if form.is_valid():
            waiting_list_entry = form.save(commit=False)  # Don't commit yet
            waiting_list_entry.user = request.user  # Ensure the logged-in user is set
            waiting_list_entry.event = event  # Set the event for this waiting list entry
            waiting_list_entry.save()  # Save the entry
            return redirect('waiting_list_success', event_id=event.id)  # Redirect to the success page
        else:
            messages.error(request, "There was an error joining the waiting list. Please try again.")
    else:
        form = WaitingListForm(initial={'event': event})  # Pass event in initial form data
...
def user_dashboard(request):
    ...
    # Fetch registrations and waiting list entries specific to the logged-in user
    user_registrations = Registration.objects.filter(user=user).select_related('event')
    user_waiting_list = WaitingList.objects.filter(user=user).select_related('event')

    # Debugging output (you can remove this after testing)
    print(f"User: {user.username}, Registrations: {user_registrations.count()}, Waiting List: {user_waiting_list.count()}")
